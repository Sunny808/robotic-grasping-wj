...
from utils.visualisation.plot import save_results, plot_incorrect_grasp_results
...
def parse_args():
    ...
    parser.add_argument('--dataset-path', type=str, default = 'cornell',
...
if __name__ == '__main__':
    ...
    save_folder = os.path.join('results', args.network[0])
            ...
            for idx, (x, y, didx, rot, zoom) in enumerate(test_data):
                ...
                if args.iou_eval:
                    ...
                    if s:
                        ...
                        plot_incorrect_grasp_results(
                            fig = fig,
                            rgb_img = test_data.dataset.get_rgb(didx, rot, zoom, normalise = False),
                            depth_img = test_data.dataset.get_depth(didx, rot, zoom),
                            grasp_q_img = q_img,
                            grasp_angle_img = ang_img,
                            ground_truth_bbs = test_data.dataset.get_gtbb(didx, rot, zoom),
                            no_grasps = args.n_grasps,
                            grasp_width_img = width_img
                        )
                        fig.savefig(os.path.join(save_folder, 'img_result_{}.pdf'.format(didx)))
                        plt.close(fig)
                    else:
                        results['failed'] += 1
