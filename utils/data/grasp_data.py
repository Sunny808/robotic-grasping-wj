...
class GraspDatasetBase(torch.utils.data.Dataset):
    ...
    def __init__(self, output_size=224, include_depth=False, include_rgb=True, random_rotate=False,
                 random_zoom=False, input_only=False):# 修改
        """
    ...
    def __getitem__(self, idx):
        ...
        # Load the grasps
        bbs = self.get_gtbb(idx, rot, zoom_factor) # 这里返回的是GraspRectangles类，其中默认参数grs是GraspRectangle类

        pos_img, ang_img, width_img = bbs.draw((self.output_size, self.output_size))
        width_img = np.clip(width_img, 0.0, self.output_size / 2) / (self.output_size / 2)# 像是进行归一化处理
