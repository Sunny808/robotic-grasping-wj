...
from utils.dataset_processing import grasp, image, test_grasp
...
class CornellDataset(GraspDatasetBase):
    ...
    def get_gtbb(self, idx, rot=0, zoom=1.0):
        gtbbs = grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx]) # return GraspRectangles类
...
class TestCornellDataset(GraspDatasetBase):
    """
    Dataset wrapper for the Cornell dataset.
    """
    ...
    def __init__(self, file_path, ds_rotate=0, **kwargs):
        ...
        :param kwargs: kwargs for GraspDatasetBase
        """
        super(TestCornellDataset, self).__init__(**kwargs)
    ...
    def _get_crop_attrs(self, idx):
        gtbbs = test_grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx])
        center = gtbbs.center# return 1X2 array
        left = max(0, min(center[1] - self.output_size // 2, 640 - self.output_size))
        top = max(0, min(center[0] - self.output_size // 2, 480 - self.output_size))
        return center, left, top
    ...
    def get_gtbb(self, idx, rot=0, zoom=1.0):
        gtbbs = test_grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx]) # return GraspRectangles类
        center, left, top = self._get_crop_attrs(idx)
        gtbbs.rotate(rot, center)
        gtbbs.offset((-top, -left))
        gtbbs.zoom(zoom, (self.output_size // 2, self.output_size // 2))
