...
def plot_results(
    ...
    if depth_img is not None:
        ...
        plot = ax.imshow(depth_img, cmap='gray', vmin=0, vmax = 1)
        ax.set_title('Depth')
        ax.axis('off')
        plt.colorbar(plot)
...
def save_results(rgb_img, grasp_q_img, grasp_angle_img, depth_img=None, no_grasps=1, grasp_width_img=None):
    ...
    :return:
    """
    gs = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps)# 返回一个Grasp
...
def plot_incorrect_grasp_results(
        fig,
        rgb_img,
        grasp_q_img,
        grasp_angle_img,
    ...
    :param grasp_q_img: Q output of network
    :param grasp_angle_img: Angle output of network
    :param ground_truth_bbs: Ground truth.
    :param no_grasps: Maximum number of grasps to plot
    :param grasp_width_img: (optional) Width output of network
    :return:
    """
    gs = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps)
    grs = ground_truth_bbs
